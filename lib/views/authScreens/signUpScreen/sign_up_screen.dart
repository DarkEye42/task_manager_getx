    ...
    if (mounted) {
      AppSnackBar().showSnackBar(
        title: status
            ...
            : AppStrings.registrationFailureTitle,
        content: status
            ? AppStrings.registrationSuccessMessage
            : AppStrings.registrationFailureMessage,
        contentType: status ? ContentType.success : ContentType.failure,
        color: status
            ? AppColor.snackBarSuccessColor
            : AppColor.snackBarFailureColor,
        context: context,
      );
      if (status) gotoSignIn();
    }
  }

  void gotoSignIn() {
    Get.find<AuthViewModel>().isPasswordObscured = true.obs;
    Get.back();
  }
